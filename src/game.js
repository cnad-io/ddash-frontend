...
class GameScene extends Phaser.Scene {
  constructor() {
    ...
    this.timedEvent
    this.timedEvent1
    //this.socket = io(window.gameRoomUrl)
    // window.gameRoomSocket.on('connect', e => { 
    //   console.log("Game Instance Connected to Socket io" )
    //  })
  ...
  create() {
    ...
    this.remoteGameData = []
    window.playerList.forEach(user  => {
      console.log("Creating remote player: "+ user.nickname);
      if (user.nickname !== window.player.nickname ){
        console.log("yes!, creating a remote player: "+ user.nickname);
        ...
        let remotePlayerLabel = this.add.text(0 , 0 , user.nickname, { fontSize: '30px', fill: '#FFFFFF' })
  
        this.remoteGameData.push({
          remotePlayer : remotePlayer,
          remotePlayerLabel : remotePlayerLabel
        })
      }else{
        console.log("Nope!")
      }
    ...
    this.currentPosition.x = this.player.x
    this.currentPosition.y = this.player.y
  ...
  update() {
    this.remoteGameData.forEach(user  => {
      user.remotePlayerLabel.x = Math.floor(user.remotePlayer.x + user.remotePlayer.width / 2)-70;
      user.remotePlayerLabel.y = Math.floor(user.remotePlayer.y + user.remotePlayer.height / 2)-80;
    })
    if (this.isGameOver === false) {
      ...
      if (this.currentPosition.x != window.player.x || this.currentPosition.y != window.player.y){
        // .emit("player moved", JSON.stringify(window.player));
        window.gameRoomSocket.emit("player moved", window.player);
        this.currentPosition.x = window.player.x
        this.currentPosition.y = window.player.y
      }
    }
