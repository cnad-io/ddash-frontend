class TitleScene extends Phaser.Scene {
  constructor() {
    ...
    window.gameRoomSocket = io.connect(window.gameRoomUrl)
    window.gameRoomSocket.on('connect', e => { 
      console.log("Title Game Scene Connected to Socket io" )
      window.gameRoomSocket.on("game ready", g => {
        console.log("Game Ready!, we have "+ window.playerList.length +" in the game room, yay, ooosom!")
        this.scene.start('GameScene')
      });
      window.gameRoomSocket.emit("join game room", window.player);
    })
    console.log("We are here, now I'm letting them know I am ready to play")
  }
